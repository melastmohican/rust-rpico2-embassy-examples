//! # Pervasive Displays E2417QS0A3 E-Paper Example (`epdsi`, async / Embassy)
//!
//! Async counterpart to `rust-rpico2-discovery`'s `pdi_e2417qs0a3` example — full parity: same
...
//!
//! Uses the same `Peri::reborrow()` pin-reuse technique as `epdsi_pdi_e2154qs0f1` (see that
//! example's module doc for the full explanation) to drive GPIO18/19 first as a bit-banged OTP
//! handshake, then as the real hardware SPI peripheral.
//!
//! Note: unlike the BW/BWR Pervasive family, the BWRY/Spectra-4 reference driver has no
//! fast/partial refresh path — only a full update. `PervasiveBwryController` reflects this:
//! there is no `PervasiveBwryRefreshMode` to select, `write_frame`/`refresh` always perform a
//! full update.
//!
//! ## Hardware
//!
...
/// Adapts [`embassy_rp::gpio::Flex`] to [`epdsi::bus3::DynamicPin`]. See
/// `epdsi_pdi_e2154qs0f1`'s module doc for why this needs no type-state enum, unlike the
/// blocking example's `FlexPin`.
...
/// Converts a 1bpp `PageBuffer` slice (400x300 bits, 15,000 bytes) into a 2bpp BWRY frame buffer
/// (400x300 2bpp, 30,000 bytes). 1bpp White (bit=1) -> 2bpp `0b01` (White); 1bpp Black (bit=0) ->
/// 2bpp `0b00` (Black).
...
async fn main(_spawner: Spawner) {
    ...
    // Hardware Note for EXT3-1 extension boards: J3 jumper must be CLOSED (47 µH path) for
    // panels > 3.7" such as this 4.2" E2417QS0A3 — the opposite setting from the smaller BWRY
    // panels — see the module doc.
    ...
    let mut epd = EpdBuilder::<_, E2417QS0A3>::new(controller).build(epd_bus);

    info!("Initializing E2417QS0A3 display (BWRY Driver A) via epdsi driver...");
    ...
    // refresh() below only issues the Display Refresh command, not a Power On.
    ...
    // The 4.2" E2417QS0A3 panel is 400x300 with no RAM overscan — the whole canvas is visible.
    ...
    Text::new(
        "epdsi PervasiveBwryController Driver A (async)",
        Point::new(10, 275),
        text_style,
    )
    .draw(&mut display)
    ...
    info!("Sending BWRY frame buffer data (30,000 bytes) to display...");
    epd.write_frame(ColorChannel::BlackWhite, &buffer_2bpp)
        .await
        .unwrap();

    info!("Refreshing display hardware (Full update — BWRY has no partial/fast refresh)...");
