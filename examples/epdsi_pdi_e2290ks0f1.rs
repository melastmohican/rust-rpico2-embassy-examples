//! # Pervasive Displays E2290KS0F1 E-Paper Example (`epdsi`, async / Embassy)
//!
//! Async counterpart to `rust-rpico2-discovery`'s `pdi_e2290ks0f1` example — full parity: same
//! panel, same wiring, same two-phase content (normal full refresh, then a 5-iteration fast
//! differential loop, each iteration re-sleeping the panel), built against `epdsi`'s async API
...
//! `write_fast_frame`; nothing else about the driver-level code differs from the blocking
//! example.
//!
//! BUSY is active-**LOW** on this panel (`Pull::Up`) — the opposite of `epdsi_pdi_e2266ks0c1`'s
//! active-high BUSY, despite both being `PervasiveBwController` panels.
//!
//! ## Hardware
//!
...
async fn main(_spawner: Spawner) {
    info!("Program start (epdsi PervasiveBwController Driver F, async/Embassy)");

    let p = embassy_rp::init(Default::default());
    let mut delay = Delay;

    // Hardware Note for EXT3-1 extension boards: J3 jumper must be OPEN (10 µH path) for panels
    // <= 3.7" such as this 2.9" E2290KS0F1 — see the module doc.
    ...
    info!("Initializing E2290KS0F1 display (Driver F) via epdsi driver...");
    ...
    info!("Initializing display registers before normal update...");
    driver.init(&mut delay).await.unwrap();
