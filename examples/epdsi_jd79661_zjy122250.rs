//! # Good Display ZJY122250-0213AJH-E5 E-Paper Example (`epdsi`, async / Embassy)
//!
//! Async counterpart to `rust-rpico2-discovery`'s `jd79661_zjy122250_epd` example — same panel,
//! same wiring, same drawing code and driver call sequence, built against `epdsi`'s async API
//! (`default-features = false, features = ["graphics"]`) over `embassy-rp`'s async SPI and GPIO,
//! instead of the blocking `rp235x-hal` API. Every `epdsi` call below is `.await`ed; the local
//! `QuadColorBuffer`/`QuadColor` 2bpp framebuffer type is unchanged — it never touches `epdsi` or
//! the HAL directly, so there's nothing in it to port.
//!
...
//! - **Display:** ZJY122250-0213AJH-E5 2.13" 4-Color (Black/White/Yellow/Red) E-Paper Display (122x250)
...
async fn main(_spawner: Spawner) {
    info!("Starting JD79661 ZJY122250 2.13\" Quad-Color EPD example (async/Embassy)");
    ...
    info!("Initializing JD79661 epdsi EPD driver (async)...");
    ...
    Text::new("epdsi async", Point::new(11, 240), text_style)
        .draw(&mut display)
        .unwrap();

    info!("Sending 8,000-byte QuadColor 2bpp frame via epdsi (async)...");
    ...
    info!("JD79661 QuadColor epdsi demo finished successfully!");
...
pub static PICOTOOL_ENTRIES: [hal::binary_info::EntryAddr; 4] = [
    hal::binary_info::rp_program_name!(c"epdsi_jd79661_zjy122250"),
    hal::binary_info::rp_program_description!(c"epdsi async JD79661/ZJY122250 example for RP2350"),
